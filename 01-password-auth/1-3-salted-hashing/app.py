"""
01-password-auth / 1-3-salted-hashing — SHA-256 with per-user salt.

DO NOT use this pattern in production.

Control added this step: unique random salt per user.
Marks W3 MITIGATED; keeps W1/W2 mitigated; leaves the rest open (no multi-jump).
"""

from datetime import timedelta
import sqlite3
from pathlib import Path
import os, hashlib  # W1/W2/W3: one-way digest + per-user salt; still fast (W4)

from flask import Flask, redirect, request, session, url_for

# ---------------------------------------------------------------------------
# Weaknesses in this step (1-3-salted-hashing) — full inventory for track 01
...
# W3  MITIGATED — unique random salt per user (same password → different digest)
...
def init_db():
    """Create users table if missing. salt is stored in the clear (normal for W3)."""
    with get_db() as conn:
        conn.execute(
            """
...
def index():
    ...
    return (
        "<h1>1-3 salted hashing (W3 mitigated)</h1>"
...
def register():
    """
    Create account.
    W3: new random salt, then SHA-256(salt || password).
    W4: still fast SHA-256.
    ...
    try:
        with get_db() as conn:
            # W1/W2/W3: salted one-way digest. W4: still fast SHA-256.
            salt = os.urandom(16)
...
def login():
    ...
    W3: load that user's salt, then same hash as register.
    ...
    # W1/W2/W3: same salted SHA-256 as register. W6: no lockout / rate limit.
